...
!define VERSION "${APPMAJORVERSION}.${APPMINORVERSION}.${BUILDNUMBER} ${APPVERSIONSTRING}"
!define INSTALL_FILENAME "KISS IDE ${APPMAJORVERSION}.${APPMINORVERSION}.${BUILDNUMBER} ${APPVERSIONSTRING}"

!define PLATFORMSTRING "KISS Platform ${VERSION}"

!define KISS_PLATFORM_DIR "C:\MinGW\msys\1.0\home\kipr\projects\kiss-platform"
...
Section "KISS Computer Target" KISSCOMPUTERTARGET
	; Set Section properties
	SetOverwrite on  ; yes, we want to overwrite existing files
	SectionIn RO     ; no, we don't want people to be able to uncheck this section   
	...
	SetOutPath "$INSTDIR\"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\KISS.exe"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\libgcc_s_dw2-1.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\mingwm10.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\QtCore4.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\QtWebKit4.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\QtNetwork4.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\QtGui4.dll"
	File "${KISS_PLATFORM_DIR}\common\package\KISS\qscintilla2.dll"
...
!insertmacro MUI_DESCRIPTION_TEXT ${KISSCOMPUTERTARGET} "KISS Computer Target"
